...
export default function AdminImagensPage() {
  ...
  async function uploadImage(file: File) {
    ...
    try {
      ...
      if (data.success) {
        // Atualiza produto selecionado com nova imagem
        ...
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className="text-sm text-[#5A7A68] hover:text-[#4A6B5A] font-medium"
            >
              Sair
            </button>
          </div>
          ...
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1">
              <input
                ...
                className="w-full px-4 py-2 border border-[#E8DFD5] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#6B8E7A]"
              />
            </div>
                  ...
                  {products.map((product) => (
                    <li key={product.id}>
                      <button
                        ...
                        className={`w-full px-4 py-3 text-left hover:bg-[#F0E8DF] transition-colors ${
                          selectedProduct?.id === product.id ? 'bg-[#F0F5F2]' : ''
                        }`}
                      >
                ...
                <div>
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {selectedProduct.product_images.map((img, idx) => (
                          <div key={img.id} className="relative aspect-square bg-[#F0E8DF] rounded overflow-hidden group">
                            ...
                            {idx === 0 && (
                              <span className="absolute top-1 left-1 text-xs bg-[#6B8E7A] text-white px-1 rounded">
                                Principal
                              </span>
                            )}
                          </div>
                  ...
                  <div
                    ...
                    className={`border-2 border-dashed rounded-lg p-8 text-center transition-colors ${
                      dragOver 
                        ? 'border-[#6B8E7A] bg-[#F0F5F2]' 
                        : 'border-[#E8DFD5] hover:border-[#6B8E7A]'
                    }`}
                  >
                    {uploading ? (
                      ...
                      <>
                        ...
                        <label className="inline-block px-4 py-2 bg-[#6B8E7A] text-white rounded-lg cursor-pointer hover:bg-[#5A7A68] transition-col
